...
def login_as(context, user_email):
	Actor(context).go_to(context.webUrls.landing_page).\
		into(EMAIL_INPUT_FIELD).type(user_email).\
		into(PASSWORD_INPUT_FIELD).type(context.content_manager_password if user_email == context.content_manager_email else context.user.password).\
