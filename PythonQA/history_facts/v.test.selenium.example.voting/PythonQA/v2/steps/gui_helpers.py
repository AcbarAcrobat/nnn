...
def wait_text(context, locator, text):
	try:
		...
		e.msg = 'A timeout occured while waiting for text "{}" to appear in element {}'.format(text, locator)
		e.screen = save_screenshot(context)
		raise
...
def wait_window_close(context, expected_window_count=1, timeout=20, interval=0.5):
	retries = 0
	while len(context.browser.window_handles) != expected_window_count:
		sleep(interval)
		retries += 1
		if retries == timeout / interval:
			context.browser.switch_to.window(context.browser.window_handles[expected_window_count])
			save_screenshot(context)
			break
