...
class API:
	...
	def check_vortex.com_in_private_index(self, session, query, entity_id):
		...
		while True:
			sleep(0.2)
			url = self.urls.private_search + 'ballotselect=id&channelselect=id&q={}&entityType=ballot%2Cchannel'.format(query)
			r = ApiRequest(session, 'get', url)
			r.perform()
	...
	def check_draft_in_private_index(self, session, author_id):
		...
		while True:
			sleep(0.2)
			url = self.urls.private_drafts + 'channel?channelId={}'.format(author_id)
			r = ApiRequest(session, 'get', url)
			r.perform()
