...
class TestWirecardBuy:
    ...
    def test_buy(self, direction, env):
        ...
        self.pay_page.data_input_wirecard(env, direction)
    ...
    def test_metabase(self, env):
        ...
        assert self.payments_object.payment_euro_check(test_last_three_transactions, 'Wirecard')
        assert self.payments_object.refund_euro_check(test_refund)
        assert self.payments_object.euro_logs_check(test_last_logs, 'wirecard')
