...
#     playbook-library/hooks/hosts_feel_gitlab.yml \
...
if [ $deploy_environment_security_configuration == 'pci' ]; then
    ...
    # Deploy primary the k8s-stack.yml from template on target database cluster

    function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
        playbook-library/\!_A_Deploy_pipeline/k8s-cluster-deploy-playbook.yml \
        -u $username --become-user root --become -e HOSTS="kubernetes-master" \
    ...
    # Deploy the k8s-stack.yml from template on target database cluster

    function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
        playbook-library/\!_A_Deploy_pipeline/docker-stack-deploy-playbook_pci_database.yml \
    ...
    # INIT ALL DATABASES 
        ...
        function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
            playbook-library/hooks/init_dbs_pci.yml \
            -e HOSTS="postgres-database[0]" -u $username --become-user root \
            --become -e "ansible_become_pass='$password'" -e "ansible_ssh_pass='$password'" -e stage="1"

    echo -e "     ${GREEN}|>..........................................................................................................................................................................................<|${NC}"

    # Deploy the k8s-stack.yml from template on target application cluster

    function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
        playbook-library/\!_A_Deploy_pipeline/docker-stack-deploy-playbook_pci_front.yml \
    ...
    # @ PERFORM MIGRATIONS
        ...
        function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
            playbook-library/hooks/init_mig_pci.yml \
            -e HOSTS="nginx-frontend[0]" -u $username --become-user root \
            --become -e "ansible_become_pass='$password'" -e "ansible_ssh_pass='$password'" -e stage="1"

    echo -e "     ${GREEN}|>..........................................................................................................................................................................................<|${NC}"

    # Deploy the k8s-vpn-stack.yml from template on target application cluster

    function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
        playbook-library/\!_A_Deploy_pipeline/docker-stack-deploy-playbook_pci_vpn.yml \
    ...
    if [ $deploy_environment_security_configuration == 'minimal' ]; then
        ...
        function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
            playbook-library/\!_A_Deploy_pipeline/docker-stack-deploy-playbook.yml \
            -u $username --become-user root --become -e HOSTS="cloud-bind-frontend-dns" \
            -e "ansible_become_pass=$password" -e "ansible_ssh_pass=$password" -e stage="docker-stack-deploy" -e version_ansible_build_id="${version_ansible_build_id}" -e "@../.local/current_tags.yml"
        ...
        function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
            playbook-library/hooks/init_dbs_all.yml \
            -e HOSTS="nginx-frontend[0]" -u $username --become-user root \
            --become -e "ansible_become_pass='$password'" -e "ansible_ssh_pass='$password'" -e stage="1"
        ...
        function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
            playbook-library/hooks/init_db_mongo.yml \
            -e HOSTS="nginx-frontend[0]" -u $username --become-user root \
            --become -e "ansible_become_pass='$password'" -e "ansible_ssh_pass='$password'" -e stage="1"
        ...
        function_selector_runner ansible-playbook -i inventories/products/$product/$inventory/ \
            playbook-library/hooks/init_db_flexy_guard.yml \
            -e HOSTS="nginx-frontend[0]" -u $username --become-user root \
            --become -e "ansible_become_pass='$password'" -e "ansible_ssh_pass='$password'" -e stage="1"
